...
```{r}
with(mtcars, bin_dist(data.frame(wt, mpg), data.frame(sample(wt), mpg)), X.bin = 5, Y.bin = 5)
```
...
```{r}
library(ggplot2)
opt.diff <- opt_diff(lineup(null_permute('mpg'), mtcars, pos = 10), var = c('mpg', 'wt'), 2, 10, 2, 10, 10, plot = TRUE, progress.bar = FALSE)
head(opt.diff$dat)
opt.diff$p
```
...
```{r}
#decrypt('...') 
#[1] 'True data in position 16' # Use pos = 16
```

```{r, message = FALSE}
dist.vals <- distmet(lineup.dat, var = c('mpg', 'wt'), 'bin_dist', null_permute('mpg'), pos = 16, dist.arg = list(X.bin = 5, Y.bin = 5), progress.bar = FALSE)
```
