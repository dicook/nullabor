...
```{r}
library(ggplot2)
opt.diff <- opt_diff(lineup(null_permute('mpg'), mtcars, pos = 10), var = c('mpg', 'wt'), 2, 10, 2, 10, 10, plot = TRUE)
head(opt.diff$dat)
opt.diff$p
```
...
```{r, message = FALSE}
dist.vals <- distmet(lineup.dat, var = c('mpg', 'wt'), 'bin_dist', null_permute('mpg'), pos = 16, dist.arg = list(X.bin = 5, Y.bin = 5))
```
